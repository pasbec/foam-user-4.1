...
void Foam::faMesh::clearGeom() const
{
    ...
    deleteDemandDrivenData(correctPatchPointNormalsAnglePtr_);
}
...
:
    ...
    correctPatchPointNormalsAnglePtr_(NULL),
...
Foam::scalar Foam::faMesh::correctPatchPointNormalsAngle(const label patchID) const
{
    if ((patchID < 0) || (patchID >= boundary().size()))
    {
        FatalErrorIn
        (
            "bool correctPatchPointNormalsAngle(const label patchID) const"
        )   << "patchID is not in the valid range"
            << abort(FatalError);
    }

    if (correctPatchPointNormalsAnglePtr_)
    {
        return (*correctPatchPointNormalsAnglePtr_)[patchID];
    }
...
//- Set patch point normals corrections
Foam::scalarList& Foam::faMesh::correctPatchPointNormalsAngle() const
{
    if (!correctPatchPointNormalsAnglePtr_)
    {
        correctPatchPointNormalsAnglePtr_ =
            new scalarList(boundary().size(), 90.0);
    }

    return *correctPatchPointNormalsAnglePtr_;
}
