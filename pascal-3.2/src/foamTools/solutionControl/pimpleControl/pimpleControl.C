...
defineTypeNameAndDebug(Foam::pimpleControl, 0);
...
bool Foam::pimpleControl::criteriaSatisfied()
{
    ...
    const dictionary& solverDict = data_.solverPerformanceDict();
    forAllConstIter(dictionary, solverDict, iter)
    {
        ...
        if (fieldI != -1)
        {
            ...
            const scalar residual = readScalar(sp.last().lookup("initialResidual"));
