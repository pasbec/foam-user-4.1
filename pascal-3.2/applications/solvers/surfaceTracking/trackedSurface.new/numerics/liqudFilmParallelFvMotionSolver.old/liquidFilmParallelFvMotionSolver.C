...
:
    ...
    trackedSurfaceZoneID_(mesh.faceZones().findZoneID("trackedSurfaceZone")),
    procToGlobalFZmap_()
{
    ...
    if(trackedSurfacePatchID_ == -1)
    {
        ...
        (
            ...
            << "Surface patch not defined." << abort(FatalError);
    }

    // Check surface face zone
    if(trackedSurfaceZoneID_ == -1)
    {
        FatalErrorIn
        (
            "liquidFilmParallelFvMotionSolver(...)"
        )
            << "Surface face zone not defined." << abort(FatalError);
    }

    const vectorField& points = fvMesh_.points();

    vectorField fzLocalPoints =
        mesh.faceZones()[trackedSurfaceZoneID_]().localPoints();
...
Foam::liquidFilmParallelFvMotionSolver::curPoints() const
{
    ...
    const labelList fzMeshPoints =
        fvMesh_.faceZones()[trackedSurfaceZoneID_]().meshPoints();
...
//             localPoint
//           < fvMesh_.boundaryMesh()[trackedSurfacePatchID_].localPoints().size()
        )
        {
    ...
    const vectorField fzLocalPoints =
        fvMesh_.faceZones()[trackedSurfaceZoneID_]().localPoints();
