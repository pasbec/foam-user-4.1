    ...
    motionDirection_(lookup("motionDirection"))
{
    ...
    {
        if(fvMesh_.boundary()[patchI].name() == "trackedSurface")
        {
            aPatchID = patchI;

            Info<< "Found surface patch. ID: " << aPatchID
                << endl;
        }
    }
    ...
    forAll (fvMesh_.boundary(), patchI)
    {
        if(fvMesh_.boundary()[patchI].name() == "trackedSurfaceShadow")
        {
            bPatchID = patchI;

            Info<< "Found surface shadow patch. ID: " << bPatchID
                << endl;
        }
    }
