...
template<class Type>
void patchDirectionMixedMixedFvPatchField<Type>::checkNHat()
{
    scalarField magNHat(Foam::mag(nHat_));

    if (min(magNHat) < SMALL)
    {
        FatalErrorIn("void patchDirectionMixedMixedFvPatchField<Type>::checkNHat()")
...
// Map from self
template<class Type>
void patchDirectionMixedMixedFvPatchField<Type>::autoMap
(
...
template<class Type>
void patchDirectionMixedMixedFvPatchField<Type>::rmap
(
...
)
{
    ...
    const patchDirectionMixedMixedFvPatchField<Type>& dmptf =
        refCast<const patchDirectionMixedMixedFvPatchField<Type> >(ptf);
...
template<class Type>
tmp<Field<Type> > patchDirectionMixedMixedFvPatchField<Type>::snGrad() const
{
...
template<class Type>
void patchDirectionMixedMixedFvPatchField<Type>::evaluate()
{
    if (!this->updated())
    {
        this->updateCoeffs();
...
template<class Type>
tmp<Field<Type> > patchDirectionMixedMixedFvPatchField<Type>::valueInternalCoeffs
(
...
//  evaluation of the value of this patchField
template<class Type>
tmp<Field<Type> > patchDirectionMixedMixedFvPatchField<Type>::valueBoundaryCoeffs
(
    const tmp<scalarField>&
) const
{
...
template<class Type>
tmp<Field<Type> >
patchDirectionMixedMixedFvPatchField<Type>::gradientInternalCoeffs() const
{
    const scalarField& deltaCoeffs = this->patch().deltaCoeffs();
    const Field<Type> one(this->size(), pTraits<Type>::one);
    const tensorField nn= nHat_*nHat_;
...
template<class Type>
tmp<Field<Type> >
patchDirectionMixedMixedFvPatchField<Type>::gradientBoundaryCoeffs() const
{
    const scalarField& deltaCoeffs = this->patch().deltaCoeffs();
    const tensorField nn= nHat_*nHat_;
