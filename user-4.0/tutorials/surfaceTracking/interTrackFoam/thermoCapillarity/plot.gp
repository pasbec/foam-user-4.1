...
!cat log | grep "fInf =" | awk '{print sqrt($7*$7)}' > finf.dat
!cat log | grep "fInf =" | awk '{print $13}' > Uinf.dat
!cat log | grep "fInf =" | awk '{print sqrt($45*$45)}' > Finf.dat
!cat log | grep "fInf =" | awk '{print sqrt($51*$51)}' > FinfVol.dat
...
plot 0 w l ls -1 notitle, \
     ...
     "Finf.dat" u ($1*1e+9) w l lw 2 t "F_{tot, A} / 1000" axes x1y1, \
     "FinfVol.dat" u ($1*1e+9) w l lw 2 dt 3 t "F_{tot, V} / 1000" axes x1y1, \
     "finf.dat" u ($1*1e+12) w l lw 2 t "F_{inf}" axes x1y1, \
     "Uinf.dat" u ($1*-1e+3) w l lw 2 t "U_{inf}" axes x1y2